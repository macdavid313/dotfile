...
**** Common Lisp
Doom Emacs somehow configures =org-babel-lisp-eval= variable to be =sly-eval=.
I'm changing it back to the default:
...
(defun acl-doc ()
  (interactive)
  (eww-browse-url "https://franz.com/support/documentation/current/doc/index.htm"))

(defun cltl2 ()
  "Quickly access Common Lisp the Language 2nd Edition"
  (interactive)
  (eww-browse-url "https://www.cs.cmu.edu/Groups/AI/html/cltl/clm/index.html"))
...
AllegroCL has its own dedicated [[https://franz.com/support/documentation/current/doc/eli.htm][Emacs lisp interface]].

The first =cond= form tries to load /eli/ form either systemed installed site or
AllegroCL installation folder.

#+begin_src emacs-lisp
(cond
 ;; load from franz workstation environment
 ((file-exists-p "/usr/fi/emacs-lib/fi/")
  (load-file (concat "/usr/fi/emacs-lib/fi/" "fi-site-init.el")))
 ;; load from installed ACL folder
 ((or (getenv "ACL_HOME") (getenv "ACL_SMP_HOME"))
  (let* ((lisp-home (file-name-as-directory (or (getenv "ACL_HOME") (getenv "ACL_SMP_HOME"))))
         (eli-directory (file-name-as-directory (concat lisp-home "eli"))))
    (when (file-exists-p eli-directory)
      (load-file (concat eli-directory "fi-site-init.el"))))))

(when (fboundp 'fi:common-lisp)
  ;; paredit
  (add-hook 'fi:common-lisp-mode-hook #'enable-paredit-mode)
  ;; rainbow
  (advice-add 'fi:common-lisp-mode :after #'rainbow-delimiters-mode-enable)
  ;; code format
  (local-set-key (kbd "M-q") 'fi:indent-sexp)
  ;; normalize evil keymaps
  (when (modulep! :editor evil +everywhere)
    (add-hook 'fi:inferior-common-lisp-mode-hook #'evil-normalize-keymaps))
  ;; display in a different window
  (setq fi:display-buffer-function #'project-display-buffer)
  ;; ALISP
  (defun fi:alisp (smp)
    (interactive (list (y-or-n-p "SMP? ")))
    (let* ((lisp-home (file-name-as-directory (getenv (if smp "ACL_SMP_HOME" "ACL_HOME")))))
      (fi:common-lisp (concat "*alisp" (if smp "-smp" "") "*")
                      fi:common-lisp-directory
                      (getenv "PWD")
                      (concat lisp-home "alisp")
                      fi:common-lisp-image-arguments
                      fi:common-lisp-host
                      (concat lisp-home "alisp.dxl"))))
  ;; MLISP
  (defun fi:mlisp (smp)
    (interactive (list (y-or-n-p "SMP? ")))
    (let* ((lisp-home (file-name-as-directory (getenv (if smp "ACL_SMP_HOME" "ACL_HOME")))))
      (fi:common-lisp (concat "*mlisp" (if smp "-smp" "") "*")
                      fi:common-lisp-directory
                      (concat lisp-home "mlisp")
                      fi:common-lisp-image-arguments
                      fi:common-lisp-host
                      (concat lisp-home "mlisp.dxl"))))
  ) ;; end of when
#+end_src
...
(use-package! slime
  ...
  :config
  (let ((hyperspec-root (file-name-as-directory (concat (getenv "HOME") "/lisp/HyperSpec"))))
    (when (file-exists-p hyperspec-root)
      (setq hyperspec-root (concat "file://" hyperspec-root))
      (setq common-lisp-hyperspec-root hyperspec-root
            common-lisp-hyperspec-symbol-table (concat hyperspec-root "Data/Map_Sym.txt")
            common-lisp-hyperspec-issuex-table (concat hyperspec-root "Data/Map_IssX.txt"))))
